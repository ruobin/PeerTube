...
var USERS_CONSTRAINTS_FIELDS = constants_1.CONSTRAINTS_FIELDS.USERS;
...
function isUserUsernameValid(value) {
    var max = USERS_CONSTRAINTS_FIELDS.USERNAME.max;
    var min = USERS_CONSTRAINTS_FIELDS.USERNAME.min;
...
function isUserDisplayNameValid(value) {
    return value === null || (misc_1.exists(value) && validator.isLength(value, constants_1.CONSTRAINTS_FIELDS.USERS.NAME));
}
exports.isUserDisplayNameValid = isUserDisplayNameValid;
function isUserDescriptionValid(value) {
    return value === null || (misc_1.exists(value) && validator.isLength(value, constants_1.CONSTRAINTS_FIELDS.USERS.DESCRIPTION));
}
...
var nsfwPolicies = lodash_1.values(constants_1.NSFW_POLICY_TYPES);
...
function isUserBlockedReasonValid(value) {
    return value === null || (misc_1.exists(value) && validator.isLength(value, constants_1.CONSTRAINTS_FIELDS.USERS.BLOCKED_REASON));
}
...
var avatarMimeTypes = constants_1.CONSTRAINTS_FIELDS.ACTORS.AVATAR.EXTNAME
    .map(function (v) { return v.replace('.', ''); })
    .join('|');
var avatarMimeTypesRegex = "image/(" + avatarMimeTypes + ")";
