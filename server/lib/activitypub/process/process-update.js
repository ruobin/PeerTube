"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
exports.__esModule = true;
var database_utils_1 = require("../../../helpers/database-utils");
var logger_1 = require("../../../helpers/logger");
var initializers_1 = require("../../../initializers");
var actor_1 = require("../../../models/activitypub/actor");
var video_channel_1 = require("../../../models/video/video-channel");
var actor_2 = require("../actor");
var videos_1 = require("../videos");
var videos_2 = require("../../../helpers/custom-validators/activitypub/videos");
var cache_file_1 = require("../../../helpers/custom-validators/activitypub/cache-file");
var cache_file_2 = require("../cache-file");
var utils_1 = require("../send/utils");
var playlist_1 = require("../playlist");
function processUpdateActivity(options) {
    return __awaiter(this, void 0, void 0, function () {
        var activity, byActor, objectType, byActorFull, byActorFull;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    activity = options.activity, byActor = options.byActor;
                    objectType = activity.object.type;
                    if (objectType === 'Video') {
                        return [2 /*return*/, database_utils_1.retryTransactionWrapper(processUpdateVideo, byActor, activity)];
                    }
                    if (!(objectType === 'Person' || objectType === 'Application' || objectType === 'Group')) return [3 /*break*/, 2];
                    return [4 /*yield*/, actor_1.ActorModel.loadByUrlAndPopulateAccountAndChannel(byActor.url)];
                case 1:
                    byActorFull = _a.sent();
                    return [2 /*return*/, database_utils_1.retryTransactionWrapper(processUpdateActor, byActorFull, activity)];
                case 2:
                    if (!(objectType === 'CacheFile')) return [3 /*break*/, 4];
                    return [4 /*yield*/, actor_1.ActorModel.loadByUrlAndPopulateAccountAndChannel(byActor.url)];
                case 3:
                    byActorFull = _a.sent();
                    return [2 /*return*/, database_utils_1.retryTransactionWrapper(processUpdateCacheFile, byActorFull, activity)];
                case 4:
                    if (objectType === 'Playlist') {
                        return [2 /*return*/, database_utils_1.retryTransactionWrapper(processUpdatePlaylist, byActor, activity)];
                    }
                    return [2 /*return*/, undefined];
            }
        });
    });
}
exports.processUpdateActivity = processUpdateActivity;
// ---------------------------------------------------------------------------
function processUpdateVideo(actor, activity) {
    return __awaiter(this, void 0, void 0, function () {
        var videoObject, video, channelActor, updateOptions;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    videoObject = activity.object;
                    if (videos_2.sanitizeAndCheckVideoTorrentObject(videoObject) === false) {
                        logger_1.logger.debug('Video sent by update is not valid.', { videoObject: videoObject });
                        return [2 /*return*/, undefined];
                    }
                    return [4 /*yield*/, videos_1.getOrCreateVideoAndAccountAndChannel({ videoObject: videoObject.id, allowRefresh: false })];
                case 1:
                    video = (_a.sent()).video;
                    return [4 /*yield*/, videos_1.getOrCreateVideoChannelFromVideoObject(videoObject)];
                case 2:
                    channelActor = _a.sent();
                    updateOptions = {
                        video: video,
                        videoObject: videoObject,
                        account: actor.Account,
                        channel: channelActor.VideoChannel,
                        overrideTo: activity.to
                    };
                    return [2 /*return*/, videos_1.updateVideoFromAP(updateOptions)];
            }
        });
    });
}
function processUpdateCacheFile(byActor, activity) {
    return __awaiter(this, void 0, void 0, function () {
        var cacheFileObject, video, exceptions;
        var _this = this;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    cacheFileObject = activity.object;
                    if (!cache_file_1.isCacheFileObjectValid(cacheFileObject)) {
                        logger_1.logger.debug('Cache file object sent by update is not valid.', { cacheFileObject: cacheFileObject });
                        return [2 /*return*/, undefined];
                    }
                    return [4 /*yield*/, videos_1.getOrCreateVideoAndAccountAndChannel({ videoObject: cacheFileObject.object })];
                case 1:
                    video = (_a.sent()).video;
                    return [4 /*yield*/, initializers_1.sequelizeTypescript.transaction(function (t) { return __awaiter(_this, void 0, void 0, function () {
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0: return [4 /*yield*/, cache_file_2.createOrUpdateCacheFile(cacheFileObject, video, byActor, t)];
                                    case 1:
                                        _a.sent();
                                        return [2 /*return*/];
                                }
                            });
                        }); })];
                case 2:
                    _a.sent();
                    if (!video.isOwned()) return [3 /*break*/, 4];
                    exceptions = [byActor];
                    return [4 /*yield*/, utils_1.forwardVideoRelatedActivity(activity, undefined, exceptions, video)];
                case 3:
                    _a.sent();
                    _a.label = 4;
                case 4: return [2 /*return*/];
            }
        });
    });
}
function processUpdateActor(actor, activity) {
    return __awaiter(this, void 0, void 0, function () {
        var actorAttributesToUpdate, accountOrChannelInstance, actorFieldsSave, accountOrChannelFieldsSave, avatarInfo, err_1;
        var _this = this;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    actorAttributesToUpdate = activity.object;
                    logger_1.logger.debug('Updating remote account "%s".', actorAttributesToUpdate.url);
                    return [4 /*yield*/, actor_2.getAvatarInfoIfExists(actorAttributesToUpdate)];
                case 1:
                    avatarInfo = _a.sent();
                    _a.label = 2;
                case 2:
                    _a.trys.push([2, 4, , 5]);
                    return [4 /*yield*/, initializers_1.sequelizeTypescript.transaction(function (t) { return __awaiter(_this, void 0, void 0, function () {
                            var avatarOptions;
                            return __generator(this, function (_a) {
                                switch (_a.label) {
                                    case 0:
                                        actorFieldsSave = actor.toJSON();
                                        if (actorAttributesToUpdate.type === 'Group')
                                            accountOrChannelInstance = actor.VideoChannel;
                                        else
                                            accountOrChannelInstance = actor.Account;
                                        accountOrChannelFieldsSave = accountOrChannelInstance.toJSON();
                                        return [4 /*yield*/, actor_2.updateActorInstance(actor, actorAttributesToUpdate)];
                                    case 1:
                                        _a.sent();
                                        if (!(avatarInfo !== undefined)) return [3 /*break*/, 3];
                                        avatarOptions = Object.assign({}, avatarInfo, { onDisk: false });
                                        return [4 /*yield*/, actor_2.updateActorAvatarInstance(actor, avatarOptions, t)];
                                    case 2:
                                        _a.sent();
                                        _a.label = 3;
                                    case 3: return [4 /*yield*/, actor.save({ transaction: t })];
                                    case 4:
                                        _a.sent();
                                        accountOrChannelInstance.name = actorAttributesToUpdate.name || actorAttributesToUpdate.preferredUsername;
                                        accountOrChannelInstance.description = actorAttributesToUpdate.summary;
                                        if (accountOrChannelInstance instanceof video_channel_1.VideoChannelModel)
                                            accountOrChannelInstance.support = actorAttributesToUpdate.support;
                                        return [4 /*yield*/, accountOrChannelInstance.save({ transaction: t })];
                                    case 5:
                                        _a.sent();
                                        return [2 /*return*/];
                                }
                            });
                        }); })];
                case 3:
                    _a.sent();
                    logger_1.logger.info('Remote account %s updated', actorAttributesToUpdate.url);
                    return [3 /*break*/, 5];
                case 4:
                    err_1 = _a.sent();
                    if (actor !== undefined && actorFieldsSave !== undefined) {
                        database_utils_1.resetSequelizeInstance(actor, actorFieldsSave);
                    }
                    if (accountOrChannelInstance !== undefined && accountOrChannelFieldsSave !== undefined) {
                        database_utils_1.resetSequelizeInstance(accountOrChannelInstance, accountOrChannelFieldsSave);
                    }
                    // This is just a debug because we will retry the insert
                    logger_1.logger.debug('Cannot update the remote account.', { err: err_1 });
                    throw err_1;
                case 5: return [2 /*return*/];
            }
        });
    });
}
function processUpdatePlaylist(byActor, activity) {
    return __awaiter(this, void 0, void 0, function () {
        var playlistObject, byAccount;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    playlistObject = activity.object;
                    byAccount = byActor.Account;
                    if (!byAccount)
                        throw new Error('Cannot update video playlist with the non account actor ' + byActor.url);
                    return [4 /*yield*/, playlist_1.createOrUpdateVideoPlaylist(playlistObject, byAccount, activity.to)];
                case 1:
                    _a.sent();
                    return [2 /*return*/];
            }
        });
    });
}
